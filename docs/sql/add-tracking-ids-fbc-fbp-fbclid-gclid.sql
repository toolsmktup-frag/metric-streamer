-- ═══════════════════════════════════════════════════════════════════
-- MIGRATION: Adicionar colunas de tracking IDs (fbc, fbp, fbclid, gclid)
-- Rodar no SQL Editor do Supabase Dashboard
-- ═══════════════════════════════════════════════════════════════════

-- 1. Colunas em ticto_transactions
ALTER TABLE public.ticto_transactions ADD COLUMN IF NOT EXISTS fbc text;
ALTER TABLE public.ticto_transactions ADD COLUMN IF NOT EXISTS fbp text;
ALTER TABLE public.ticto_transactions ADD COLUMN IF NOT EXISTS fbclid text;
ALTER TABLE public.ticto_transactions ADD COLUMN IF NOT EXISTS gclid text;

-- 2. Colunas em customer_purchases (Guru/Eduzz)
ALTER TABLE public.customer_purchases ADD COLUMN IF NOT EXISTS fbc text;
ALTER TABLE public.customer_purchases ADD COLUMN IF NOT EXISTS fbp text;
ALTER TABLE public.customer_purchases ADD COLUMN IF NOT EXISTS fbclid text;
ALTER TABLE public.customer_purchases ADD COLUMN IF NOT EXISTS gclid text;

-- 3. Recriar v_all_sales com as 4 novas colunas
