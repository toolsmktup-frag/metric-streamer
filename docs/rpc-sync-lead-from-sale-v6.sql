-- ============================================================
-- RPC: sync_lead_from_sale  (v6 — deduplicação de eventos)
--
-- Mudanças v6:
--   - Dedup de lead_events: antes de cada INSERT, verifica se
--     já existe evento com mesmo lead + funnel + event_name +
--     transaction_id (via metadata->>'transaction_id')
--   - Se transaction_id for NULL ou vazio, insere normalmente
--     (compatibilidade retroativa)
--   - Mantém toda lógica v5 (stage_transition_rules, multi-funnel)
--
...
BEGIN
  ...
  -- Extrair transaction_id do metadata para dedup
  v_txn_id := p_metadata->>'transaction_id';
  ...
  -- ─── 4. Log lead_event na BASE DE LEADS (com dedup) ───
  ...
  IF p_product_name IS NOT NULL AND p_product_name <> '' THEN
    ...
    LOOP
      ...
      -- Log event no funil do produto (com dedup)
      IF v_txn_id IS NOT NULL AND v_txn_id <> '' THEN
        IF NOT EXISTS (
          SELECT 1 FROM lead_events
          WHERE lead_id = v_lead_id
            AND funnel_id = v_prod_funnel_id
            AND event_name = p_event_name
            AND metadata->>'transaction_id' = v_txn_id
        ) THEN
          INSERT INTO lead_events (lead_id, funnel_id, event_name, metadata)
...
COMMENT ON FUNCTION public.sync_lead_from_sale IS
  'v6: Deduplicação de eventos via transaction_id no metadata. Evita inserção duplicada de lead_events quando webhooks disparam múltiplas vezes para a mesma transação. Mantém stage_transition_rules (v5) e multi-funnel routing (v4).';
