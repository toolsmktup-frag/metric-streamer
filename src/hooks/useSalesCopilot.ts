    ...
    try {
      ...
      try {
        ...
        if (streamErr?.name === 'AbortError') {
          ...
          console.error('[useSalesCopilot] stream interrupted:', streamErr);
          if (gotAnyContent) {
            toast.warning('Resposta interrompida. Mostrando o que chegou.');
          } else {
            toast.error('Conexão com o copiloto caiu. Tente de novo.');
          }
        }
      }

      if (!gotAnyContent) {
        toast.warning('Copiloto não retornou resposta. Tente novamente.');
      }
    } catch (e: any) {
